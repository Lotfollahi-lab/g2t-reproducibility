#!/usr/bin/env bash
# Download LUNA's MERFISH-mouse-cortex preprocessed CSVs from Google Drive.
# This is the LUNA Figure 3 dataset (Zhang et al. 2021 cortex, Mouse 1 train
# / Mouse 2 test).
#
# Source URL (LUNA-team Google Drive, single-file link):
#   https://drive.google.com/file/d/1j6W5NZV56_W3kO_UxXEtFv1nIlwUksLi/view?usp=drive_link
#
# Expected output (under $DEST_DIR after extraction):
#   MERFISH_mouse_cortex_train.csv  - Mouse 1 (33 slices, 158k cells)
#   MERFISH_mouse_cortex_test.csv   - Mouse 2 (31 slices, 118k cells)
#
# Defaults (override via env vars):
#   DEST_DIR = /nfs/team361/sb75/DATASETS/bronze/mmc_luna
#   FILE_ID  = 1j6W5NZV56_W3kO_UxXEtFv1nIlwUksLi
#
# Note: this is the same file id referenced as the "sample cortex"
# download in `download_luna_paper_csvs.sh`, but this script writes it
# directly into the canonical bronze location and integrates with the
# downstream build_h5ad_from_luna_csv.py pipeline.
#
# Usage:
#   bash scgg-reproducibility/analysis/benchmarking/download_mmc_luna_csvs.sh
#   DEST_DIR=/some/other/path bash .../download_mmc_luna_csvs.sh

set -euo pipefail

DEST_DIR="${DEST_DIR:-/nfs/team361/sb75/DATASETS/bronze/mmc_luna}"
FILE_ID="${FILE_ID:-1j6W5NZV56_W3kO_UxXEtFv1nIlwUksLi}"

log() { printf '[download_mmc_luna_csvs] %s\n' "$*"; }
...
# ---------------------------------------------------------------------------
STAGE_DIR="$(mktemp -d -t mmc_luna_dl.XXXXXX)"
...
WANT_TRAIN="$DEST_DIR/MERFISH_mouse_cortex_train.csv"
WANT_TEST="$DEST_DIR/MERFISH_mouse_cortex_test.csv"
log ""
...
log "        --out_dir   /nfs/team361/sb75/DATASETS/silver/mmc_luna \\"
log "        --prefix    merfish_mouse_cortex --overwrite"
